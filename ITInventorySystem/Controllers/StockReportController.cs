...
public class StockReportController(IReportInterface reportService) : ControllerBase
{
    ...
    public async Task<IActionResult> DownloadStockMovementReport([FromQuery] DateTime startDate,
    ...
    /// </summary>
    [HttpGet("work-orders")]
    public async Task<IActionResult> DownloadWorkOrderReport([FromQuery] DateTime startDate,
                                                             [FromQuery] DateTime endDate)
    {
        if (startDate > endDate)
            return BadRequest(new { message = "A data de início não pode ser maior que a data de fim." });
